#!/usr/bin/env bash
#
# Build, install and verify ba-ddd-mapper on a local cluster.
#
#   ./helm/ba-ddd-mapper/deploy.sh              # build, upgrade, restart, test
#   ./helm/ba-ddd-mapper/deploy.sh --no-build   # skip the image build
#   ./helm/ba-ddd-mapper/deploy.sh --no-test    # skip helm test
#
# Environment overrides:
#   NAMESPACE  (default ba-hub)
#   RELEASE    (default ba-ddd-mapper)
...
NAMESPACE="${NAMESPACE:-ba-hub}"
RELEASE="${RELEASE:-ba-ddd-mapper}"
...
SOURCE_DIR="$REPO_ROOT/ba-ddd-mapper"
